...
class User extends BaseModel implements Transformable, AuthenticatableContract, CanResetPasswordContract
{
    ...
    public function setUp(CompanyRepositoryEloquent $companyRepo,
        ...
        )
    {
        ...
        $typeService = $typeRepo->create(array('entity_key' => 'entry',
            'name' => 'repair',
            'company_id' => $company->id)
            );
    
        $typeRepair = $typeRepo->create(array('entity_key' => 'entry',
            'name' => 'service',
            'company_id' => $company->id)
            );
        ...
        $typeTour = $typeRepo->create(array('entity_key' => 'trip',
            'name' => 'tour',
            'company_id' => $company->id)
            );
    
        $typeDelivery = $typeRepo->create(array('entity_key' => 'trip',
            'name' => 'delivery',
            'company_id' => $company->id)
            );
        ...
        $contactDriver = $contactRepo->create(array('company_id' => $company->id,
        ...
        $modelTruck = $modelRepo->create(array('model_type_id' => $typeTruck->id,
            'vendor_id' => $contactVendor->id,
            'name' => 'Generic Truck',
            'company_id' => $company->id)
            );
    
        $vehicleTruck = $vehicleRepo->create(array('model_vehicle_id' => $modelCar->id,
            'number' => 'IOP-1234',
            'initial_miliage' => 123,
            'actual_miliage' => 123,
            'cost' => 50000,
